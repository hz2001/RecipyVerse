...
const ProfilePage = () => {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      setIsLoggedIn(true)
      setIsMerchant(stored.is_merchant)
    } else {
      setShowRegisterModal(true)
    }
  }, [])
  ...
  if (!isLoggedIn) {
    return (
        <div className="p-8">
          <h1 className="text-2xl font-bold mb-4">Welcome to RecipeVerse</h1>
      ...
      <div className="p-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-4">
            <img
                src={`https://avatars.dicebear.com/api/identicon/${currentUser?.wallet_address}.svg`}
                alt="avatar"
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">{isMerchant ? 'Merchant' : 'User'}</p>
            </div>
          </div>
        ...
        <div>
          <h3 className="text-lg font-semibold mb-2">My NFTs / Recipes</h3>
          <p className="text-sm text-gray-500">You can fetch user's data from Supabase here.</p>
        </div>
      </div>
  )
}
